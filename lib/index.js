...
exports.createMbtiles = async (mbtilesPath, opts) => {
  ...
  if (currentId && currentId !== dirNode.cid.toString()) {
  ...
  if (currentId !== dirNode.cid.toString()) {
    debug(`publish new directory /ipfs/${dirNode.cid.toString()} under /ipns/${key.id}`)
    await ipfs.name.publish(dirNode.cid.toString(), { key: key.name })
  }
...
exports.deploy = async (opts) => {
  ...
  debug('new ipfs address', result.cid.toString())

  let key = (await ipfs.key.list()).find(k => k.name === 'ipfs-tiles')
  if (!key) key = await ipfs.key.gen('ipfs-tiles', { type: 'rsa', size: 2048 })
  await ipfs.name.publish(result.cid.toString(), { key: key.name })
  console.log(`published /ipns/${key.id}`)
}
